...
- **Avoid em dashes entirely.** They read as an AI tell. Use commas, colons, or periods instead.
...
- **Machine-readable:** real on-page text, never PDFs or images for facts; structured data;
  one consistent name.
- **Wire every new post into all four:** the blog index (`blog/index.html`, holding a card *and* the
...
- **The frame is three layers:** SEO (ranking), AEO (the answer box), GEO (being named inside
  an AI answer).
- **The metric that matters is whether the machine names us**, not vanity traffic.
